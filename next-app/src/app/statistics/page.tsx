...
import {
  URL_LOCATILY_STATS,
  URL_LOGIN_STATS,
  URL_RESET_PASSWORD_STATS,
  URL_SIGNUP_STATS,
  URL_SNAP_STATS,
  URL_SNAPS_PER_MONTH,
  URL_USER_STATS,
  URL_USERS_PER_MONTH,
} from '../user/commun/urls';
...
export default function StatisticsPage() {
  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      const user_stats = await fetch_async(URL_USER_STATS, 'identity');
  ...
  useEffect(() => {
    const fetchLocationStats = async () => {
      const locations_stats = await fetch_async(URL_LOCATILY_STATS, 'identity');
  ...
  useEffect(() => {
    const fetchLogInStats = async () => {
      const logInStats = await fetch_async(URL_LOGIN_STATS, 'identity');
  ...
  const [completeSignUpSuccessfulRate, setCompleteSignUpSuccessfulRate] =
    useState(0);
  const [completeSignUpErrorRate, setCompleteSignUpErrorRate] = useState(0);

  useEffect(() => {
    const fetchSignUpStats = async () => {
      const signUpStats = await fetch_async(URL_SIGNUP_STATS, 'identity');
      ...
      setCompleteSignUpSuccessfulRate(
        signUpStats.complete_sign_up_successful_rate,
      );
  ...
  const signUp = [
    ...
    {
      name: 'Complete Sign Up Successful Rate',
      value: completeSignUpSuccessfulRate,
    },
  ...
  const [resetPasswordSuccessfulRate, setResetPasswordSuccessfulRate] =
    useState(0);
  const [resetPasswordErrorRate, setResetPasswordErrorRate] = useState(0);

  useEffect(() => {
    const fetchResetPasswordStats = async () => {
      const resetPasswordStats = await fetch_async(
        URL_RESET_PASSWORD_STATS,
        'identity',
      );
      ...
      setResetPasswordSuccessfulRate(
        resetPasswordStats.reset_password_successful_rate,
      );
  ...
  const resetPassword = [
    ...
    {
      name: 'Reset Password Successful Rate',
      value: resetPasswordSuccessfulRate,
    },
  ...
  useEffect(() => {
    const fetchSnapstats = async () => {
      const user_stats = await fetch_async(URL_SNAP_STATS, 'content');
  ...
  const [totalSnapsPerMonth, setTotalSnapsPerMonth] = useState<
    { month: string; value: number }[]
  >([]);

  useEffect(() => {
    const fetchNewUsersStats = async () => {
      const new_snaps_per_month_stats = await fetch_async(
        URL_SNAPS_PER_MONTH,
        'content',
      );
  ...
  useEffect(() => {
    const fetchNewUsersStats = async () => {
      const new_users_per_month_stats = await fetch_async(
        URL_USERS_PER_MONTH,
        'identity',
      );
      ...
      >
        <div className="mt-6 sm:mt-2 2xl:mt-5">
          ...
          <Tab.Panels>
            ...
            <Tab.Panel>
              <GrowthStats
                snaps_per_month={snaps_per_month}
